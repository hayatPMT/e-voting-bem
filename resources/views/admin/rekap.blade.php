...
<div class="modern-card">
    ...
    <div class="table-responsive">
        <table class="table table-modern">
            ...
            <tbody>
                @php
                    $totalVotes = $data->sum('votes_count');
                    $maxVotes = $data->max('votes_count');
                @endphp

                @forelse ($data as $index => $kandidat)
                    <tr class="{{ $kandidat->votes_count == $maxVotes && $maxVotes > 0 ? 'winner-row' : '' }}">
                        ...
                        <td>
                            ...
                            @if($kandidat->votes_count == $maxVotes && $maxVotes > 0)
                        ...
                        <td class="text-center">
                            @if ($totalVotes > 0)
                                <span class="badge-modern badge-percentage">
                                    ...
                                    {{ round(($kandidat->votes_count / $totalVotes) * 100, 2) }}%
                                </span>
                            @else
...
<script>
    @if($data->count() > 0 && $totalVotes > 0)
    const labels = @json($data->pluck('nama'));
    const dataValues = @json($data->pluck('votes_count'));
