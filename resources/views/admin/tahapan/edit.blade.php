...
<div class="row mb-4">
 <div class="col-12 text-center">
  <h1 class="h3 font-weight-bold text-gray-800">Edit Tahapan Pemilihan</h1>
  <p class="text-muted small">Perbarui data dan jadwal tahapan pemilihan.</p>
 </div>
</div>
    ...
    <form action="{{ route('admin.tahapan.update', $tahapan->id) }}" method="POST">
     ...
     <div class="form-group mb-4">
      ...
      <div class="input-group input-group-merge border rounded-lg overflow-hidden shadow-sm">
       ...
       <input type="text" name="nama_tahapan" id="nama_tahapan" class="form-control border-0 pl-2 py-4" placeholder="Contoh: Masa Kampanye" value="{{ old('nama_tahapan', $tahapan->nama_tahapan) }}" required autofocus>
      </div>
      @error('nama_tahapan')
     ...
     <div class="form-group mb-4">
      ...
      <div class="input-group input-group-merge border rounded-lg overflow-hidden shadow-sm">
       ...
       <textarea name="deskripsi" id="deskripsi" class="form-control border-0 pl-2 py-3" rows="3" placeholder="Jelaskan detail aktivitas pada tahapan ini...">{{ old('deskripsi', $tahapan->deskripsi) }}</textarea>
      </div>
     </div>
      ...
      <div class="col-md-6">
       <div class="form-group mb-4">
        ...
        <div class="input-group input-group-merge border rounded-lg overflow-hidden shadow-sm">
         ...
         <input type="datetime-local" name="waktu_mulai" id="waktu_mulai" class="form-control border-0 pl-2 py-4" value="{{ old('waktu_mulai', $tahapan->waktu_mulai ? $tahapan->waktu_mulai->format('Y-m-d\TH:i') : '') }}" required>
        </div>
        @error('waktu_mulai')
        <small class="text-danger font-weight-bold mt-1 d-block"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</small>
        @enderror
      ...
      <div class="col-md-6">
       <div class="form-group mb-4">
        ...
        <div class="input-group input-group-merge border rounded-lg overflow-hidden shadow-sm">
         ...
         <input type="datetime-local" name="waktu_selesai" id="waktu_selesai" class="form-control border-0 pl-2 py-4" value="{{ old('waktu_selesai', $tahapan->waktu_selesai ? $tahapan->waktu_selesai->format('Y-m-d\TH:i') : '') }}" required>
        </div>
        @error('waktu_selesai')
        <small class="text-danger font-weight-bold mt-1 d-block"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</small>
        @enderror
