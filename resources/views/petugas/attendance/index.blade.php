...
@section('content')
        ...
        <div class="col-md-6">
            @if ($activeBoothId)
                ...
                <div class="alert alert-info py-2 mb-0">
                    <i class="fas fa-desktop mr-2"></i> Bilik Aktif:
                    <strong>{{ $currentBooth->nama_booth ?? 'Unknown' }}</strong>
                </div>
            @else
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control form-control-lg" id="nim_search"
                                placeholder="Contoh: 12345678" autofocus>
                    ...
                    <div id="search_result" class="d-none animate__animated animate__fadeIn">
                        ...
                        <form action="{{ route('petugas.attendance.approve') }}" method="POST">
                            ...
                            <div class="form-group">
                                <label for="mode_select" class="font-weight-bold mb-2">Mode Voting:</label>
                                <select name="mode" id="mode_select" class="form-control form-control-lg" required>
                                    <option value="">-- Pilih Mode --</option>
                                    <option value="offline" selected>
                                        <i class="fas fa-desktop"></i> Offline (Di TPS)
                                    </option>
                                    <option value="online" disabled
                                        title="Online students harus self-register melalui fitur online voting">
                                        <i class="fas fa-globe"></i> Online (Self-Register)
                                    </option>
                                </select>
                                <small class="form-text text-muted mt-2">
                                    Mahasiswa online harus melakukan konfirmasi kehadiran sendiri melalui sistem voting
                                    online.
                                </small>
                            </div>

                            <button type="submit" class="btn btn-success btn-block btn-lg shadow">
                                <i class="fas fa-check-circle mr-2"></i> APPROVE & TERBITKAN TOKEN (OFFLINE)
                            </button>
                        </form>
                    </div>
                        ...
                        <div id="token_display" class="mt-4 animate__animated animate__bounceIn">
                            ...
                            <div id="token_card_print" class="card border-success shadow-sm">
                                <div class="card-body text-center py-4">
                                    <h6 class="text-uppercase font-weight-bold text-success mb-3"><i
                                            class="fas fa-ticket-alt mr-2"></i>TOKEN VOTING BERHASIL DITERBITKAN</h6>
                                    <div class="token-box mb-3">
                                        <div class="token-code">{{ session('generated_token') }}</div>
                                    </div>
                                    <p class="mb-3 text-muted">Berikan 6 digit kode unik di atas kepada mahasiswa.<br>Kode
                                        ini dapat digunakan di bilik suara mana saja.</p>
                                    <div class="btn-group">
                                        <button onclick="window.print()" class="btn btn-dark btn-print-hide">
                                            <i class="fas fa-print mr-1"></i> Cetak Token
                                        </button>
                                        <button onclick="document.getElementById('token_display').classList.add('d-none')"
                                            class="btn btn-outline-secondary btn-print-hide">
                                            Tutup
                                        </button>
                                    </div>
                                </div>
                        ...
                        <tbody>
                            @forelse($attendances as $attendance)
                                <tr>
                                    ...
                                    <td><code
                                            class="font-weight-bold text-primary">{{ $attendance->session_token ?? '-' }}</code>
                                    </td>
                                    <td>
                                        @if ($attendance->mode == 'online')
                                            <span class="badge badge-info"><i class="fas fa-globe mr-1"></i> Online</span>
                                        @else
                                            <span class="badge badge-secondary"><i class="fas fa-desktop mr-1"></i>
                                                Offline</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($attendance->status == 'approved')
                                            <span class="badge badge-warning text-dark"><i class="fas fa-clock mr-1"></i>
                                                Waiting Vote</span>
    ...
    <!-- Booth Setup Modal -->
    <div class="modal fade" id="boothSetupModal" tabindex="-1" aria-labelledby="boothSetupModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
                <div class="modal-header bg-info text-white" style="border-radius: 15px 15px 0 0;">
                    <h5 class="modal-title font-weight-bold" id="boothSetupModalLabel"><i class="fas fa-cog mr-2"></i>
                        Konfigurasi Bilik Suara</h5>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('petugas.attendance.setBooth') }}" method="POST"
                            ...
                            <select name="voting_booth_id" class="form-control" required>
                                ...
                                @foreach ($votingBooths as $booth)
                                    <option value="{{ $booth->id }}"
                                        {{ (int) $activeBoothId == $booth->id ? 'selected' : '' }}>
                                        {{ $booth->nama_booth }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <small class="text-muted mt-2 d-block">Bilik yang dipilih akan menjadi pilihan otomatis saat anda
                            melakukan approval.</small>
                    </form>
                    ...
                    <div class="list-group">
                        @foreach ($votingBooths as $booth)
                            <a href="{{ route('voting-booth.standby', $booth->id) }}" target="_blank"
                                class="list-group-item list-group-item-action d-flex justify-content-between align-items-center border-left-info">
                                <div>
                                    <strong class="text-dark">{{ $booth->nama_booth }}</strong>
                                    <br>
                                    <small class="text-muted">{{ $booth->lokasi ?? 'Lokasi Umum' }}</small>
                                </div>
                                <span class="btn btn-sm btn-outline-info rounded-pill"><i
                                        class="fas fa-external-link-alt mr-1"></i> Buka</span>
                            </a>
                        @endforeach
                    </div>
                </div>
