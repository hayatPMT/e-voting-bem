            ...
            <div class="hero-content text-center">
                @if (isset($setting) && $setting->election_logo)
                    <div class="hero-icon p-0 overflow-hidden border-0" style="background: transparent;">
                        <img src="{{ asset('storage/' . $setting->election_logo) }}" alt="Logo"
                ...
                <h4 class="mb-3" style="font-weight:700; color:var(--gold);">
                    {{ $kampus->nama ?? 'Semua Kampus' }}
                </h4>
                <p
                    style="font-size: 1.125rem; opacity: 0.95; margin-bottom: 0; font-weight: 500; max-width: 700px; margin: 0 auto;">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadData() {
                fetch(`/api/chart?kampus_id={{ $kampus->id }}`)
                    .then(res => res.json())
                    .then(data => {
                        if (chart) {
